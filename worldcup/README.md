...
1. Import this repo in Vercel and set the **Root Directory** to `worldcup`.
2. Add a **Postgres** store from the Vercel dashboard (one click — it injects
   `DATABASE_URL` automatically; no separate signup).
3. Set environment variables: `ADMIN_PASSWORD` and `CRON_SECRET`.
4. After the first deploy, run `npx prisma db push` and `npm run db:seed` against the
   production `DATABASE_URL` (e.g. via the Vercel CLI) to create and seed tables.
5. The cron in `vercel.json` calls `/api/cron/sync` automatically.

Push to `claude/world-cup-fantasy-web-C8DAO` triggers an auto-deploy.
